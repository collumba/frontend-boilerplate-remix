...
function EntityFormClient({ entity, id, isCreate = true }: EntityFormProps) {
      ...
      if (entityConfig.fields) {
        Object.entries(entityConfig.fields).forEach(([key, field]) => {
          if (field.type === 'date' && processedData[key as keyof FormValues]) {
            ...
            processedData[key as keyof FormValues] =
              typeof dateValue === 'string'
                ? dateValue
                : (dateValue as unknown as Date).toISOString();
          }
        });
      }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className={`flex gap-2 flex-wrap ${isMobile ? 'w-full' : 'w-auto'} justify-end`}>
          <Button
            type="button"
            variant="outline"
      ...
      <Form {...form}>
        <form id="entity-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  return (
                    <FormItem>
                      <FormLabel htmlFor={`field-${key}`} id={`label-${key}`}>
                        ...
                        {field.required && (
                          <span className="text-destructive ml-0.5">{REQUIRED_FIELD_MARKER}</span>
                        )}
                      </FormLabel>
                      <FormControl>
                        {field.type === 'text' || field.type === 'number' ? (
                          ...
                          <Textarea
                            ...
                            placeholder={field.placeholder ? t(field.placeholder) : undefined}
                            minLength={
                              typeof field.min === 'string' ? parseInt(field.min) : field.min
                            }
                            maxLength={
                              typeof field.max === 'string' ? parseInt(field.max) : field.max
                            }
                            disabled={field.disabled}
                            readOnly={field.readonly}
                          ...
                          <div className="multiselect-wrapper">
                            {field.optionsSource === 'api' ? (
                              ...
                              <MultiSelect
                                options={field.options || []}
                                selected={
                                  getTypedFormFieldValue(formField.value, 'multiselect') as string[]
                                }
                                onChange={formField.onChange}
                                placeholder={
                                  field.placeholder
                      ...
                      {field.helperText && (
                        <p className="text-sm text-muted-foreground">{t(field.helperText)}</p>
                      )}
                      <FormMessage />
                    </FormItem>
                  );
